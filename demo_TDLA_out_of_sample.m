...
for iDB = 1:length( Dataset_name )% DB is data base; and length(DB) is number of Data Base.
    ...
    Y = gnd;
    % =====================================================================
    type_method = 'TDLA'; % as type = 'NTD_LE' & options.alpha = 0, that is NTD algorithm
    tensor_shape    = [16 16];% the dimension for dataset dependent reshaped tensors
    lowerdims = [7 8];
    % =====================================================================
    ...
    for rn = 1 : runtimes
        ...
        for cros = 1 : cross_num
            ...
            train = X( train_indx, :);                         %===================%
            train_label = Y( train_indx, 1);                     %      选择训练集    %
            %                                                  %===================%
            ...
            % ============================== train  =======================
            R_label = litekmeans(V.data', R_nClasses,'Replicates', 20);
