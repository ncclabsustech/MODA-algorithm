function [Us, outputs, ela] = EMan_HODA(Xs, classes, lowerdims, tensor_shape)
% =========================================================================
X_N = size(Xs, 2);
...
% Define the problem cost function and its Euclidean gradient.
        problem.cost  = @(U, store) mycost(U, Rb, Rw, nmodes, store);
        function [f, store] = mycost(U, Rb, Rw, nmodes, store)
            
            store = prepare(U, Rb, Rw, nmodes, store);
            f = -trace(store.UtBU/store.UtWU);
        end
     
        problem.grad = @(U, store) mygrad(U, Rb, Rw, nmodes, store);
        function [g, store] = mygrad(U, Rb, Rw, nmodes, store)
            store = prepare(U, Rb, Rw, nmodes, store);
            g.U1 = -store.Gs{1};
            g.U2 = -store.Gs{2};
        end
